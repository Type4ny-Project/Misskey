...
export class AddMisskeyCallsDomain1783781174801 {
	...
	async up(queryRunner) {
		await queryRunner.query(`CREATE TABLE "calls_room" ("id" character varying(32) NOT NULL, "attachmentType" character varying(16) NOT NULL, "ownerUserId" character varying(32) NOT NULL, "chatRoomId" character varying(32), "title" character varying(256) NOT NULL, "description" character varying(2048) NOT NULL DEFAULT '', "visibility" character varying(16) NOT NULL DEFAULT 'specified', "visibleUserIds" character varying array NOT NULL DEFAULT '{}', "state" character varying(16) NOT NULL DEFAULT 'scheduled', "scheduledAt" TIMESTAMP WITH TIME ZONE, "startedAt" TIMESTAMP WITH TIME ZONE, "endedAt" TIMESTAMP WITH TIME ZONE, "revision" integer NOT NULL DEFAULT '0', "createdAt" TIMESTAMP WITH TIME ZONE NOT NULL, "updatedAt" TIMESTAMP WITH TIME ZONE NOT NULL, CONSTRAINT "CHK_calls_room_lifecycle" CHECK (("state" = 'scheduled' AND "startedAt" IS NULL AND "endedAt" IS NULL) OR ("state" = 'open' AND "startedAt" IS NOT NULL AND "endedAt" IS NULL) OR ("state" IN ('ended', 'cancelled') AND "endedAt" IS NOT NULL)), CONSTRAINT "CHK_calls_room_revision" CHECK ("revision" >= 0), CONSTRAINT "CHK_calls_room_state" CHECK ("state" IN ('scheduled', 'open', 'ended', 'cancelled')), CONSTRAINT "CHK_calls_room_visibility" CHECK ("visibility" IN ('public', 'followers', 'specified')), CONSTRAINT "CHK_calls_room_attachment_type" CHECK ("attachmentType" IN ('personal', 'chatRoom')), CONSTRAINT "CHK_calls_room_attachment" CHECK (("attachmentType" = 'personal' AND "chatRoomId" IS NULL) OR ("attachmentType" = 'chatRoom' AND "chatRoomId" IS NOT NULL)), CONSTRAINT "PK_acdd3afab4f4cda873dd1f07191" PRIMARY KEY ("id"))`);
