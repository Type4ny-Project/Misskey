...
import { callsApplicationId, callsMediaApiError, callsMediaErrors, sessionDescriptionParam } from '../_media-shared.js';
...
export const paramDef = { type: 'object', properties: { roomId: { type: 'string', format: 'misskey:id' }, connectionId: { type: 'string', minLength: 8, maxLength: 128 }, sessionDescription: sessionDescriptionParam }, required: ['roomId', 'connectionId'] } as const;
@Injectable()
export default class extends Endpoint<typeof meta, typeof paramDef> { // eslint-disable-line import/no-default-export
	constructor(service: CallsMediaService, credentials: CallsMediaCredentialService) {
		super(meta, paramDef, async (ps, me, token) => {
			try {
				const result = await service.createSession(me, ps);
