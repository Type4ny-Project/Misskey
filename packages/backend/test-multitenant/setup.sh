...
mkdir -p certificates .config volumes/db.a volumes/db.b
...
generate() {
  ...
  openssl x509 -req -sha256 \
    ...
    -days 500
  sed "s/\${HOST}/$1/g" .config/example.conf > .config/$1.conf
}
...
cp .config/example.config.json .config/multitenant.config.json
